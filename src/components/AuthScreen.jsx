...
export default function AuthScreen({ onAuth }) {
  ...
  return (
    <div className="d-flex align-items-center justify-content-center min-vh-100" style={{background:'linear-gradient(180deg,#343541 0%,#202123 100%)'}}>
      <div className="auth-card p-4 p-md-5 shadow-lg" style={{maxWidth:480,width:'100%',borderRadius:24,background:'rgba(255,255,255,0.08)',backdropFilter:'blur(12px)',border:'1px solid rgba(255,255,255,0.15)',minHeight:520,position:'relative'}}>
        <h2 className="mb-3 fw-bold" style={{color:'#fff',fontSize:'1.9rem'}}>TUNIA</h2>
        <p className="text-secondary mb-4" style={{fontSize:'0.95rem'}}>Sign {mode==='signup'? 'up' : 'in'} to start chatting in Tunisian Darija.</p>
        <form onSubmit={submit} className="needs-validation" noValidate>
          {mode === 'signup' && (
            <div className="mb-3">
              ...
              <div className="form-text" style={{color:'#bbb'}}>Lowercase, no spaces.</div>
            </div>
          )}
          <div className="mb-3">
            ...
            <div className="mb-3">
              ...
              <div className="form-text" style={{color:'#bbb'}}>At least 6 characters.</div>
            </div>
            {mode === 'signup' && (
              <div className="mb-3">
        ...
        <p className="text-secondary mb-0" style={{fontSize:'0.7rem'}}>By continuing you agree to basic usage logging for improving study help. No spam.</p>
      </div>
    </div>
  );
}
